...
<script>
    ...
    // Weather layer radio button functionality
    ...
    function createFullDayWeatherPopup(currentWeather, fullDayForecast, lat, lng) {
        ...
        // Calculate rain amount (matching your display logic exactly)
    ...
    async function saveForecastSnapshots(lat, lng) {
        ...
        try {
            ...
            // Prepare forecast time slots payload (matching your display cards)
            const timeSlots = {};
            ...
            console.log('Sending forecast snapshots:', postData);
            ...
            if (result.success) {
                ...
                showNotification(
                    `Forecast snapshots saved successfully. ${Object.keys(timeSlots).length} periods saved`,
                    'success'
                );
    ...
    // // Display snapshots on map as markers
    // function displaySnapshotsOnMap(snapshots) {
    //     // Clear existing snapshot markers
    //     map.eachLayer(function (layer) {
    //         if (layer.options && layer.options.isSnapshotMarker) {
    ...
    //             icon: L.divIcon({
    //                 html: `
    //                     <div class="bg-white rounded-full border-2 border-blue-500 p-1 shadow-lg">
    //                         <div class="text-xs font-bold text-blue-600">${Math.round(snapshot.temperature)}°</div>
    //                     </div>
    //                 `,
    ...
    //                 iconAnchor: [16, 16]
    //             }),
    //             isSnapshotMarker: true
    //         });

    //         marker.bindPopup(`
    //             <div class="p-2">
    //                 <h4 class="font-bold text-gray-800 mb-1">${snapshot.location}</h4>
    //                 <div class="text-sm space-y-1">
    //                     <div><strong>Temperature:</strong> ${snapshot.temperature}°C</div>
    //                     <div><strong>Time:</strong> ${snapshot.snapshot_time}</div>
    //                     <div><strong>Status:</strong> ${snapshot.storm_status}</div>
    //                 </div>
    //             </div>
    //         `);
